...
- 6 de dano direto no alvo.
- Uma explosão visual (vórtice expandindo + burst) no ponto do impacto.
- **No caso específico do escudo do chefe**: o Swirl **destrói o escudo**, não é bloqueado por ele. Ver §3.2.4.
...
Quando o Swirl atinge um chefe com escudo ativo, o escudo é **imediatamente desativado** (`enemy.isShieldActive = false`), a mesh do escudo é escondida, e o Swirl **é consumido** nesse impacto (não perfura o chefe depois de destruir o escudo — é 1 dano de 6 no chefe, e o escudo some).
