...
check('#1 Boss bloqueia dano na origem durante transition', () => {
  ...
  const direct = block(enemiesSrc, 'resolveProjectileHit(prevPos, currPos', 'resolvePiercingProjectileHits(prevPos, currPos')
  assert.ok(direct.indexOf('enemyHit.kind === BOSS_KIND && enemyHit.transitioning') < direct.indexOf('enemyHit.hp -= damage'))

  const piercing = block(enemiesSrc, 'resolvePiercingProjectileHits(prevPos, currPos', 'getEnemyCount()')
...
check('#7 homing e Swirl abandonam alvo em fadingOut', () => {
  ...
  const swirl = block(projectilesSrc, 'if (projectile.isPiercing && projectile.swirlHomingTarget)', 'if (projectile.life != null)')
