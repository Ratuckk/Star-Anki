...
import { nextQuestion, resolveAnswer, getSummary, computeDifficultyBias } from './quiz.js'
...
function mountGame(session, deck, menu) {
  ...
  const state = {
    ...
    arenaCutsceneDurationMs: 0, // setado por startArenaCutscene (arenaCutscene.js) antes de rodar
  ...
  // chefe, Fase 5). pendingQuestionKind marca qual settle usar enquanto phase === 'questionPause'.
  // v0.32: pergunta abre na hora que o ciclo termina — sem a fase 'recall' de 3.5s.
  ...
  function tick(now) {
    ...
    if (state.phase === 'combat') {
      ...
      // igual ao original (preservando a assimetria estrutural com handleBossDefeated).
      ...
      state.phaseTimer -= dt * 1000
      if (state.phaseTimer <= 0) {
        if (state.pendingCardChoice) enterCardChoice(bossFlow.resumeCombatFromGolden)
