...
> **Status (ver [`docs/progress/archive/PROGRESSO_v0.61-v0.74.md`](../../progress/archive/PROGRESSO_v0.61-v0.74.md) pro relato completo):** ✅ **Fase 1 concluída** — Blaster (§4.1) e Tank (§4.8) migrados. ⚠️ **Antes de implementar Boss (§4.4) ou Golden (§4.5)**: este documento tem pelo menos 2 números errados verificados contra o código real — duração de `PHASE_TRANSITION` documentada como 2.0s (real: 1.2s) e um vínculo inventado "transição ativa o escudo" (o escudo é um ciclo independente que na verdade pausa durante a transição) — releia o código-fonte real antes de confiar em qualquer valor específico daqui, não só nesses dois. Os demais 9 inimigos (§4.2, §4.3, §4.6, §4.7, §4.8 restante) ainda não foram verificados contra o código real.
