# Progresso pós-v0.30 — CONGELADO na v0.50.0

> **⚠️ Este arquivo está congelado.** O histórico cobre v0.34.0 → v0.50.0.
> A continuação está em [PROGRESSO_POS_0.50.md](PROGRESSO_POS_0.50.md) (por sua vez congelado na
> v0.60.0, com a continuação em [PROGRESSO_POS_.60.md](PROGRESSO_POS_.60.md) — esse é o arquivo
> ATUAL, onde toda entrega nova deve ser documentada).

Continuação do [PROGRESSO.md](PROGRESSO.md) (histórico até v0.33.x, agora congelado).
...
1. **`src/mobile.js` (novo)** — `initMobileSupport()` (chamado uma vez no boot, `main.js`) cria um overlay full-screen (`#rotate-overlay`) e escuta `resize`/`orientationchange` pra recalcular se deve aparecer. `syncRotateOverlay()` só mostra o aviso quando a tela de JOGO está de fato visível E o viewport está mais alto que largo — os menus (pré-jogo, gerenciador de baralhos, Configurações) continuam 100% usáveis na vertical, é só a partida em si que exige paisagem. `requestGameOrientation()` (chamado no início de `mountGame`) e `releaseGameOrientation()` (chamado em `teardown()`) tentam, na ordem, tela cheia (`requestFullscreen`) e travar a orientação (`screen.orientation.lock('landscape')`) — os dois são *best-effort* dentro de `try/catch`: funcionam no Android Chrome, falham silenciosamente em qualquer navegador sem suporte (majoritariamente iOS Safari, que não tem nenhum dos dois fora de um PWA instalado) — o overlay de aviso é o que garante que sempre dá pra jogar, com ou sem esse suporte extra do navegador.
2. **`hud-shared.js`** — `showScreen()` passou a chamar `syncRotateOverlay()` no fim, então toda troca de tela (entrar na partida, voltar pro menu, fim de setor) recalcula o aviso automaticamente, sem precisar espalhar essa chamada pelos vários pontos que trocam de tela.
3. **Toque genérico**: todas as telas fora da partida (pré-jogo, gerenciador de baralhos, Configurações — incluindo a seção nova de mapeamento de gamepad da v0.48.0, editor de teclado, fim de setor, prática de painel) já eram só `<button>`/clique — nenhuma dependia de teclado ou mouse-only (`:hover` é só cosmético). Confirmado revisando cada uma antes de mexer, nenhuma mudança necessária aí além do polimento abaixo.
4. **Polimento de toque (`index.html`)**: `touch-action: manipulation` em botões/links/inputs/chips — sem isso, toques rápidos repetidos (o fluxo de "Definir" botão de gamepad, por exemplo) sofrem o atraso de ~300ms do navegador esperando um possível double-tap-pra-zoom, e às vezes disparam zoom sem querer. `viewport-fit=cover` no meta viewport (aparelhos com notch). Media query nova (`max-width: 480px`) empilha `.settings-row`/`.keybind-row` (label em cima, controle embaixo) em vez de espremer tudo numa linha só — quebrava em telas estreitas (testado num viewport de 375px, ex: iPhone SE).

**Testado**: `node --check` limpo em `mobile.js`/`hud-shared.js`/`main.js` e `selftest.mjs` passou. **Testado ao vivo** com o viewport do navegador de teste emulado em 375×812 (retrato) e 812×375 (paisagem): pré-jogo e Configurações (incluindo a seção de mapeamento de gamepad) renderizam sem overflow horizontal e sem erro de console nos dois tamanhos; o `.keybind-row`/`.settings-row` empilhado confirmado visualmente nas linhas de label longo (ex: "Propulsor (impulso; segurar com Z/C no all-range = deslocar lateral)"). O aviso de girar o celular confirmado via `showScreen('game')` direto no console (sem precisar carregar um baralho de verdade): aparece em retrato com a tela de jogo ativa, desaparece ao trocar pra paisagem (`resize`) E ao voltar pra qualquer outra tela mesmo continuando em retrato (`showScreen('pregame')`) — as 2 transições confirmadas por leitura direta de `overlay.hidden`, zero erro de console em todas as trocas.

**Não verificável neste ambiente** (sem hardware): controle físico de verdade parado num celular via Bluetooth — o suporte em si (`Gamepad API`) não muda com este pedido, já foi testado com um gamepad fabricado na v0.48.0, e é a mesma API em desktop/mobile; `requestFullscreen`/`orientation.lock` de verdade pegando num Android real (o navegador de teste não expõe essas APIs do jeito que um Chrome mobile de verdade expõe) — o fallback (aviso de girar) foi o testado a fundo, exatamente por ser o caminho garantido independente de suporte do navegador.

**Versão**: v0.48.0 → v0.49.0.
