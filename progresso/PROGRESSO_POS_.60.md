# Progresso pós-v0.60 — CONGELADO na v0.74.1

> **⚠️ Este arquivo está congelado.** O histórico cobre v0.61.0 → v0.74.1.
> A continuação está em [PROGRESSO_POS_.70.md](PROGRESSO_POS_.70.md).
...
Pedido do usuário: *"prepare um overhaul de todo o design da região onde fica as vidas, saúde, escudo e boost para deixar ele mais dinamico e épico, quero opções"*.

1. **4 propostas visuais interativas apresentadas antes de codar** — Artifact publicado com 4 direções completas e clicáveis (cada uma reage a "sofrer dano"/"perder vida"/"recarregar escudo"/"ativar boost" de verdade, não é mockup estático): **Esquadrão de Elite** (placas angulares cortadas em diagonal), **Núcleo Reator** (anéis radiais tipo reator, pulso de batimento cardíaco), **Alerta de Combate** (holograma tático com scanline e glitch) e **Fúria Estelar** (barras cinéticas grossas com afterburner). Cada uma anotada com os princípios de animação clássicos (dos 12 de Disney) que ela usa de propósito — timing, arco, antecipação, continuidade, etc. — a pedido específico do usuário sobre o motivo das escolhas de animação. Usuário escolheu **Esquadrão de Elite**.
2. **Implementação no jogo real (`src/hud-game.js`)**: `livesBar`/`shieldBar`/`healthBar`/`boostBar` agora vivem dentro de um `hud-vitals-cluster` (flex-column, mesma posição de tela de sempre — canto superior esquerdo — só trocou position:absolute individual por gap consistente). Vida e escudo trocaram de barra lisa (`width: X%`) para **segmentos discretos** (`hud-seg` por unidade de vida/escudo, reconstruídos sob demanda quando `maxHealth`/`maxShield` mudam — escudo pode crescer até `SHIELD_MAX_CAP` via carta, então o rebuild dinâmico é necessário de verdade, não só defensivo). Pips de vida trocaram de triângulo simples pra losango tipo brasão, com animação de "estilhaço" (squash-then-stretch) no pip exato que é perdido. Boost continua uma barra contínua, mas ganhou corte diagonal e listras animadas de propulsão quando ativo.
3. **Feedback dinâmico novo**: dano (vida OU escudo caindo) dispara um flash de brilho de 260ms no cluster inteiro (`hit-flash`, detectado comparando com o valor do frame anterior — só dispara em decréscimo real, não em toda chamada). Vida ≤ 40% do máximo (reaproveita `LOW_HEALTH_THRESHOLD_FRAC` de `main-constants.js`, o mesmo threshold que já disparava a vinheta de tela — evita um segundo número mágico) ativa um pulso vermelho contínuo nos segmentos de vida preenchidos.
4. **API pública 100% preservada**: `setLives/setStatus/setShield/setBoost` mantêm a mesma assinatura — `game-loop.js`/`main.js` não mudaram uma linha. `.hud-bar-wrap`/`.hud-shield-fill`/`.hud-health-fill` (classes antigas) foram removidas por completo do `index.html` (CSS morto), e o overhaul migrou de vez essa região pra dentro de `hud-styles.js` (`injectHudExtraStyles`), consistente com a preferência já registrada no projeto de manter CSS de HUD lá, não solto em `index.html`. `.cinematic-active`/transição de cutscene atualizados pra apontar pro novo `.hud-vitals-cluster` em vez das 4 classes antigas individualmente.

**Testado**: `node --check` em `hud-game.js`/`hud-styles.js` e `node src/selftest.mjs`, 100% ok. **Testado ao vivo** (`preview_start "static"`, Modo Arcade, stepper de frame manual pela mesma limitação de rAF sem foco de SO já documentada): confirmado visualmente por screenshot que o cluster renderiza no canto certo com a estética de placas angulares (pips losango ciano, segmentos de escudo azuis, segmentos de vida verdes, boost âmbar com corte diagonal). Dano aplicado via painel de debug ("Causar 1 dano") 6x seguidas: contagem de segmentos verdes caiu de 10→7→4 exatamente como esperado a cada clique, e a classe `crit` ativou sozinha exatamente ao cruzar 4/10 (40%). Zero exceção no console durante toda a sequência. **Achado de ambiente, não é bug**: durante essa sessão de teste, `getBoundingClientRect()` do cluster relatou momentaneamente `top: -52px` (em vez do `top: 40px` correto) logo depois de mexer no painel de debug — sumiu sozinho após um reload limpo, com o mesmo elemento voltando a `top: 40px` exato sem nenhuma mudança de código. Bate com a mesma categoria de instabilidade de renderização do Browser pane sem foco de SO já registrada em sessões anteriores (v0.62.3/v0.63.x/v0.68.0) — não afeta o jogo real, só a própria janela de teste automatizado.

**Versão**: v0.70.1 → **v0.71.0**.
